...
#copy 11_tripletALN_input_for_RDP to working DIR in windows
#copy 7_stat DIR in windows
#set input directory  NOTE: no space in URL!!
my $workingdir='C:\Users\Duo\Dropbox\pro\Tarleton_Lab\Trans-sialidase_recomb\automate\splitNblast\3k_ts_win50\realign';
my $infiledir=$workingdir."\\11_tripletALN_input_for_RDP\\"; 
#set RDP directory
my $rdpURL="C:\\RDP4_2\\RDP4.exe";  
#OUTPUT:
...
open DETECTEDRECOMB, ">DETECTEDRECOMB_ALNTriplet.txt";
open RDPFAIL, ">RDPfail.record_ALNTriplet.txt";
opendir DIR,"11_tripletALN_input_for_RDP" or die;
system ("del 11_tripletALN_input_for_RDP\\\*\.rdp"); #remove previous generated rdp file
system ("del 11_tripletALN_input_for_RDP\\\*\.csv"); #remove previous generated csv file
#system ("11_tripletALN_input_for_RDP\\del \*\.csv");
ONEFILE: while(my $filename=readdir(DIR))
{  
next if $filename eq "." or $filename eq "..";
#start count seq number
my $seqio=Bio::SeqIO->new(-format => 'Fasta', -file=>"11_tripletALN_input_for_RDP\\$filename");
my $seqcount=0;
$total_count++;
...
$seqcount++;
	...
	if ($seqcount>2) {   ####only run RDP if more than 2 seqs is in the file####
		print "\n${total_count}. excuting RPD on $infiledir$filename\n";
		system ("$rdpURL -f $infiledir$filename -o");
		print "system return status $?  $!\n";
       if ($?==0 and (-e "11_tripletALN_input_for_RDP\\${filename}.csv")) { #### check if RDP exucted successfully and output CSV file exists
    #### excute RDP4
	
	    #######start analysis of ONE RDP4 result ######
open RDPRESULT, "11_tripletALN_input_for_RDP\\${filename}.csv";
my @file=split(/\./,$filename);
my $filequeryname;
if ($filename=~/unannotated/) {$filequeryname=$file[0];}
else {$filequeryname=$file[0].'.'.$file[1];}
#print $filequeryname;
my $line1=<RDPRESULT>;
...
closedir DIR;
#######start output result
open OUTRDP,">${workingdir}\\7_stat\\RDP_AlnTriplet.txt";
