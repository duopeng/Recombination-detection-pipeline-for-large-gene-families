################################################################################################
#This scripts get all NON-overlapping putative recombinant events for REALIGNED RDP RESULTS!!!
...
#OUTPUT >putative_recomb_events_NONoverlap_realigned.txt
###############################################################################################
#! /usr/bin/perl
use strict;
use warnings;
...
open TEMPEVENT, ">putative_recomb_events_NONoverlap_realigned.txt" or die;
...
my $gene_flag=0;
#print TEMPEVENT "\n".$filename."\n";
open DETECTEDRECOMBDETIAL, "11_tripletALN_input_for_RDP/$filename" or die "11_tripletALN_input_for_RDP/$filename";
my $filenamewithNo=$filename;
$filenamewithNo=~s/\.fasta\.aln\.fas\.csv//;
$filename=~s/\.No\d+?\.fasta\.aln\.fas\.csv//;
#print $filename."\n";
...
my $currentseqname='';
#print '11_tripletALN_input_for_RDP\\'.$filenameWithNo.'.fasta.aln.fas'."\n";
if (-e '11_tripletALN_input_for_RDP\\'.$filenameWithNo.'.fasta.aln.fas') {

my $seqio=Bio::SeqIO->new(-format => 'Fasta', -file=>'11_tripletALN_input_for_RDP\\'.$filenameWithNo.'.fasta.aln.fas');
my $seqobj;
